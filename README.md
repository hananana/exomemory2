...
| [Obsidian](https://obsidian.md) | vault 閲覧のフロントエンド（任意・推奨） |

macOS で `jq` が入っていない場合:
...
> **Note:** v0.1 までは `CLAUDE_MEMORY_VAULT` という環境変数名を使っていた。後方互換のため `EXOMEMORY_VAULT` 未設定時は `CLAUDE_MEMORY_VAULT` をフォールバックで読むが、deprecation 警告が出る。**v0.3 で `CLAUDE_MEMORY_VAULT` サポートは削除予定**。
