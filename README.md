...
v0.8 では **SessionStart orphan rescue** が入った。tmux の `prefix + x` のように Claude Code が SIGHUP で即死するケース（SessionEnd hook が走らない）でも、次のセッション起動時に未 handover の transcript と滞留した clip queue を自動検出してバックグラウンドで取り込む。`prefix + x` を含めた閉じ方ぜんぶに対して、handover 化と ingest が漏れなく走るようになった。v0.8.1 では rescue/auto-ingest が **`INGEST_BATCH_SIZE`（既定 10）で 1 回あたりの取り込みを上限制限**するようになり、初回大量取り込みや大量 dirty 蓄積時にも 1 invocation が hang しないようになった（subagent 結果による成否誤判定の修正も併せて入っている）。v0.8.2 では handover の `last_captured_at` を transcript の最初メッセージ時刻から取るようにして rescue rebuild 時の日付集中を解消、`dirty=0` のときに ingest が無駄に spawn する副次バグを修正、Handover calendar の過密日でレイアウトが崩れる問題を CSS snippet で抑える対策を入れた。`/wiki-migrate` を 1 回実行すると過去の handover の captured_at を会話時刻に backfill する。v0.8.3 では v0.8.2 で導入した CSS snippet の selector が Dataview の実 DOM と噛み合っていなかった点を実機検証で確定版に修正（`table-layout:fixed` + 多重 `!important` で過密日でも cell が崩れない）。

v0.9 では Karpathy 原典の弱点（信頼度管理なし / 古い情報の腐敗 / wikilink の意味欠如 / ingest の揺れ）を 4 軸でまとめてカバーした。**信頼度スコア**（entity / concept ページに `confidence` / `sources` / `last_verified` を導出）、**supersession**（「X から Y に移行」のような明示フレーズを ingest が拾い、旧ページに `stale: true` + `superseded_by` を立てる）、**typed Connections**（`depends_on::` / `contradicts::` / `caused_by::` / `fixed_in::` / `supersedes::` / `related_to::` の Dataview inline field 化）、**MERGE 衝突解決の決定木**（supersession → recency → authority → 両論併記の順で WIKI.md に明文化）。新 dashboard は `low-confidence` / `stale` / `contradictions` / `dependencies` の 4 件。`/wiki-query` は default で stale ページを除外するが、history キーワード（`history` / `経緯` / `以前` 等）や `[[<slug>]]` 直接参照、`supersedes::` の 1 ホップ traversal で stale も拾う機械的 retrieval rule を持つ。`/wiki-migrate` を 1 回実行すると既存 entity / concept に confidence backfill と Connection の `related_to::` プレフィックス化が一括で入る（idempotent、PyYAML 不使用の stdlib only）。v0.9.1 では `wiki/index.md` の section 順を **Concepts → Entities → Sources** に変更（抽象から具象へ降りる順、`/wiki-migrate` で既存 vault も自動 reorder。3 セクションが連続してない customized index は触らない）。
...
- [x] v0.7: ingest preflight の bash 化と batch SKIP（速度改善）
- [x] v0.8: SessionStart orphan rescue + spawn ガード + handover timestamp 修正 + calendar CSS 確定
- [x] v0.9: Karpathy 弱点対応 — confidence scoring / supersession / typed Connections / MERGE 決定木。新 dashboard 4 件、`/wiki-migrate` で既存 vault に backfill
- [ ] v0.10: `/wiki-lint`（quality scoring + self-healing）— v0.9 の confidence データが運用で蓄積されてから設計
- [ ] v1.x: decay / forgetting — vault 規模が肥大化したら検討
