...
v0.9 closes four well-documented weaknesses of Karpathy's LLM Wiki pattern (no confidence management, stale-info rot, semantically-empty wikilinks, ingest non-determinism) in one shot. **Confidence scoring** — every entity/concept page derives `confidence` / `sources` / `last_verified` from the actual wiki state. **Supersession** — explicit linguistic triggers like "switched from X to Y" / 「X から Y に移行」 mark the old page `stale: true` with `superseded_by` pointing at the replacement. **Typed Connections** — bullets in the Connections section use Dataview inline fields (`depends_on::` / `contradicts::` / `caused_by::` / `fixed_in::` / `supersedes::` / `related_to::`), turning each link into a typed edge instead of an opaque 1-bit signal. **MERGE conflict resolution** — WIKI.md ships an explicit decision tree (supersession → recency → authority → dual-statement) so re-ingesting the same source produces the same result. Four new dashboards (`low-confidence`, `stale`, `contradictions`, `dependencies`) ride on top. `/wiki-query` excludes stale pages by default but reaches them through a deterministic Bash preprocess: history keywords (`history`, `previous`, `deprecated`, `old`, plus Japanese equivalents), direct `[[<slug>]]` references, and one-hop `supersedes::` traversal all rehydrate the stale side. Run `/wiki-migrate` once to backfill confidence on existing entities/concepts and prefix bare `- [[X]]` Connection lines with `- related_to:: ` (idempotent, stdlib-only Python — no PyYAML). v0.9.1 reorders the `wiki/index.md` content sections to **Concepts → Entities → Sources** (abstract first, raw last). `/wiki-migrate` reorders existing vaults automatically; vaults with a customized index (foreign H2 between the trio) are left untouched.
...
- [x] v0.7: bash-based ingest preflight with batch SKIP (speed improvement)
- [x] v0.8: SessionStart orphan rescue + spawn guard + handover timestamp fix + finalized calendar CSS
- [x] v0.9: Karpathy weakness mitigations — confidence scoring / supersession / typed Connections / MERGE decision tree. Four new dashboards. `/wiki-migrate` backfills existing vaults.
- [ ] v0.10: `/wiki-lint` (quality scoring + self-healing) — design after v0.9 confidence data accumulates in real use
- [ ] v1.x: decay / forgetting — revisit when vaults grow large
