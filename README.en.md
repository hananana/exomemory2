...
v0.8 adds **SessionStart orphan rescue**. When Claude Code dies via SIGHUP (e.g. `tmux prefix + x` killing the pane before SessionEnd hook can fire), the next session boot detects un-handovered transcripts and stranded clip queues, then rebuilds and ingests them in the background. Captures and ingest no longer leak through any close path, including hard kills.
